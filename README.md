...
4. **Settings** → **Environment variables** に次の値を登録します。
   ...
   - `TAG_GAME_PASSWORD_HASH`: 下の「安全なパスワードハッシュの作り方」で作った値
5. Deploy を実行します。

## 安全なパスワードハッシュの作り方

Cloudflare の環境変数には、平文パスワードではなく `salt:hash` 形式の PBKDF2-SHA-256 ハッシュを保存します。
...
- 36人全員に同じ共有パスワードを配る場合、誰かが外部へ共有すると全員分の入室権限が漏れます。イベント後、または参加者が変わったら必ず `TAG_GAME_PASSWORD_HASH` を更新してください。
