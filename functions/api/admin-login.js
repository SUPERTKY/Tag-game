...
function validateAdminAuthConfig(env) {
  if (env.TAG_GAME_ADMIN_PASSWORD) return null;

  if (!env.TAG_GAME_ADMIN_PASSWORD_HASH) {
    return jsonResponse({ ok: false, error: "Admin password is not configured." }, 500);
  }

  const parts = env.TAG_GAME_ADMIN_PASSWORD_HASH.split(PASSWORD_HASH_SEPARATOR);
  if (parts.length !== 2 || !parts[0] || !parts[1]) {
    return jsonResponse({ ok: false, error: "Admin password hash is invalid." }, 500);
  }
...
async function verifyConfiguredAdminPassword(password, env) {
  if (env.TAG_GAME_ADMIN_PASSWORD) {
    return timingSafeEqualText(password, env.TAG_GAME_ADMIN_PASSWORD);
  }

  return verifyPasswordHash(password, env.TAG_GAME_ADMIN_PASSWORD_HASH);
}
